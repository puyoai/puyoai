...
class RensaTracker {
public:
  RensaTracker(RensaTrackResult* trackResult) : m_result(trackResult) {
...
CoreField::CoreField(const std::string& url) : PlainField(url) {
  heights_[0] = 0;
...
CoreField::CoreField(const CoreField& f) : PlainField(f) {
  for (int x = 0; x < MAP_WIDTH; ++x)
    heights_[x] = f.heights_[x];
}

CoreField::CoreField(const PlainField& f) : PlainField(f) {
  heights_[0] = 0;
  heights_[MAP_WIDTH - 1] = 0;
  for (int x = 1; x <= WIDTH; ++x)
    recalcHeightOn(x);
...
int CoreField::dropAfterVanish(int minHeights[], Tracker* tracker) {
  ...
  for (int x = 1; x <= WIDTH; x++) {
    ...
    DCHECK(color(x, writeAt) == PuyoColor::EMPTY)
      << writeAt << ' ' << toChar(color(x, writeAt));
