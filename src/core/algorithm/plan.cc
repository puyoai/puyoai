...
std::vector<Plan> Plan::findAvailablePlans(const CoreField& field,
  ...
  iterateAvailablePlans(
      field, kumipuyoSeq, kumipuyoSeq.size(),
      [&plans](const RefPlan& ref) { plans.push_back(ref.toPlan()); });

  return plans;
}

static void iterateAvailablePlansInternal(
...
void Plan::iterateAvailablePlans(const CoreField& field,
  ...
  iterateAvailablePlansInternal(field, kumipuyoSeq, decisions, 0,
                                maxDepth, 0, callback);
}
