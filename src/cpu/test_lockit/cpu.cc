...
FrameResponse TestLockitAI::playOneFrame(const FrameRequest& request)
{
...

    {
        ...
        if (event.grounded) {
            onGroundedForEnemy(request);
        }
        if (event.puyoErased) {
            onPuyoErasedForEnemy(request);
        }
        if ((event.decisionRequest || event.decisionRequestAgain) && r_player[1].set_puyo_once == 1) {
            onDecisionRequestedForEnemy(request);
        }
    }

    {
        ...
        if (event.grounded) {
            onGroundedForMe(request);
        }
        if (event.puyoErased) {
            onPuyoErasedForMe(request);
        }
        if ((event.decisionRequest || event.decisionRequestAgain) && r_player[0].set_puyo_once == 1) {
            onDecisionRequestedForMe(request);
            return sendmes(&r_player[0], &coma);
        }
        if (event.wnextAppeared) {
            onNext2AppearedForMe(request);
        }
    }
...
void TestLockitAI::onGroundedForEnemy(const FrameRequest&)
{
...
void TestLockitAI::onPuyoErasedForEnemy(const FrameRequest&)
{
    if (r_player[1].field.isZenkeshi()) {
        r_player[1].zenkesi = 1;
    }
}

void TestLockitAI::onDecisionRequestedForEnemy(const FrameRequest&)
{
...
void TestLockitAI::onGroundedForMe(const FrameRequest&)
{
    r_player[0].set_puyo_once = 1;
    r_player[0].keep_score = r_player[0].score;
}

void TestLockitAI::onPuyoErasedForMe(const FrameRequest&)
{
...
void TestLockitAI::onDecisionRequestedForMe(const FrameRequest&)
{
...
void TestLockitAI::onNext2AppearedForMe(const FrameRequest&)
{
    if (coma.m_hukks != 0) {
        PuyoColor field[2][6][kHeight];
