...
bool ComplementBookField::isMatchable(const CoreField& field) const
{
    ...
    return matcher.match(pattern_, field).matched;
}

bool ComplementBookField::complement(const CoreField& field, ColumnPuyoList* cpl) const
{
    PatternMatcher matcher;
    if (!matcher.match(pattern_, field).matched)
        return false;
    ...
    cpl->clear();
    for (int x = 1; x <= 6; ++x) {
        int h = pattern_.height(x);
        for (int y = 1; y <= h; ++y) {
            if (pattern_.type(x, y) != PatternType::VAR &&
                pattern_.type(x, y) != PatternType::MUST_VAR) {
                if (field.color(x, y) == PuyoColor::EMPTY)
                    return false;
                continue;
            }
            char c = pattern_.variable(x, y);
            if (!matcher.isSet(c))
                return false;
            if (ColumnPuyoList::MAX_SIZE <= cpl->size())
                return false;
            if (pattern_.type(x, y) == PatternType::MUST_VAR) {
