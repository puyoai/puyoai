...
bool Ctrl::isQuickturn(const Field& field, const KumipuyoPos& k) {
  ...
  return (field.color(k.x - 1, k.y) && field.color(k.x + 1, k.y));
}
  ...
  while(1) {
    ...
    if (start.r == 1) {
      ...
      if (isQuickturn(field, start)) {
        ...
        if (field.color(start.x - 1, start.y)) {
          add(KEY_LEFT_TURN, ret);
          add(KEY_LEFT_TURN, ret);
        } else {
          add(KEY_RIGHT_TURN, ret);
    ...
    if (goal.x == start.x) {
      switch(goal.r) {
        ...
        case 1:
          if (field.color(start.x + 1, start.y)) {
            if (field.color(start.x + 1, start.y + 1) ||
                !field.color(start.x, start.y - 1)) {
              return false;
            }
    ...
    if (goal.x > start.x) {
      ...
      if (!field.color(start.x + 1, start.y)) {
        ...
        */
        ...
        if (field.color(start.x, start.y - 1) || isQuickturn(field, start)) {
